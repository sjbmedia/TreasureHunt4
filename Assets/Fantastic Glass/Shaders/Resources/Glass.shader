		...
		_Opacity("Opacity", Range(0,1)) = 0.0
			...
			_BumpMagnitude("Bump Magnitude", float) = 1.0
				...
				[Toggle]_CapExtinctionValues("Cap Extinction Values (min,max)", int) = 1
					...
					_AberrationTexture("Aberration Texture (RGBA)", 2D) = "black" {}
				...
				_DepthColourFar("Fog Far", Color) = (0,0,0,0)
					...
					[Toggle]_FlipUV_Depth("Flip Depth UV (Forward Rendering / Platform Specific)", int) = 0
							...
							//	DEPTH TECHNIQUE 1.1
								...
								//	DEPTH TECHNIQUE 1.1
									...
									void surf(Input IN, inout SurfaceOutputStandard o)
									{
										...
										float externalDepthDiff = ((backDepth - frontDepth) * _DepthMagnitude) + _DepthOffset;

										float internalDepthDiff = ((internalBackDepth - frontDepth) * _DepthMagnitude) + _DepthOffset;
										...
										if (_EnableDistortion)
										{
											...
											#if NORMAL_WORLD_CAM_SHADER
											distortion += dot(worldNormal, _CameraNormal) * _DistortionIntensity.y;
											#endif
										...
										if (_DoubleDepthPass)
										{
											...
											externalDepthDiff = ((backDepth - frontDepth) * _DepthMagnitude) + _DepthOffset;

											internalDepthDiff = ((internalBackDepth - frontDepth) * _DepthMagnitude) + _DepthOffset;
										}
										...
										if (_EnableAberration)
										{
											...
											aberrationMagnitude = max(_AberrationMagnitude.y, min(_AberrationMagnitude.z, aberrationMagnitude));

											half3 aberration = aberrationMagnitude * distortion;

											uv_refraction_aberration_r.xyz += aberration * _Aberration.r;
											uv_refraction_aberration_g.xyz += aberration * _Aberration.g;
											uv_refraction_aberration_b.xyz += aberration * _Aberration.b;
										...
										if (_EnableFog)
										{
											float fogDepth = internalDepthDiff * _FogMagnitude / backDepth;
										...
										if (_EnableColourExtinction)
										{
											...
											//external depth used, in case of backless geometry
											float extinction = externalDepthDiff * _ColourExtinction.a * extinctionTextureColour.a * _ColourExtinctionMagnitude.x;
